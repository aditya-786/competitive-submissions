            ...
            while (testcases-- > 0) {
                ...
                if (xb == xa || yb == ya) {
                    if (xb != xa) {
                        if(ya==yb) {
                            int flag = 0;
                            for (long i = Math.min(xa, xb); i <= Math.max(xa, xb); i++) {
                                if (yf != ya || i != xf) {
                                    continue;
                                }
                                out.write((Math.abs(xa - xb) + 2 + " ").getBytes());
                                out.write(("\n").getBytes());
                                continue start;
                            }
                            out.write((Math.abs(xa - xb) + " ").getBytes());
                            out.write(("\n").getBytes());
                            continue start;
                        }
                    } else {
                        for (long i = Math.min(ya, yb); i <= Math.max(ya, yb); i++) {
                            if (xf != xa || i != yf) {
                                continue;
                            }
                            out.write((Math.abs(ya - yb) + 2 + " ").getBytes());
                            out.write(("\n").getBytes());
                            continue start;
                        }
                        out.write((Math.abs(ya - yb) + " ").getBytes());
                        out.write(("\n").getBytes());
                        continue start;
                    }
                } else {
                    out.write((Math.abs(xa - xb) + Math.abs(ya - yb) + " ").getBytes());
                    out.write(("\n").getBytes());
                    continue start;
                }
