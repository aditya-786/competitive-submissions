    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i = 0; i < k; i++) res.append(sb.charAt(i % n));
                ...
                while (i < n) {
                    ...
                    for (int j = 0; j < k; j++) temp2.append(temp.charAt(j % temp.length()));

                    StringBuilder min = new StringBuilder();
                    boolean ff = false;
                    int j = 0;
                    while (j < Math.min(temp2.length(), res.length())) {
                        if (temp2.charAt(j) < res.charAt(j)) {
                            min = new StringBuilder(temp2);
                            ff = true;
                            break;
                        } else if (temp2.charAt(j) > res.charAt(j)) {
                            min = new StringBuilder(res);
                            ff = true;
                            break;
                        }
                        j++;
