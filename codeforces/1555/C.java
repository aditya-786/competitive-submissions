    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i = 0; i < 2; i++) {
                    for (int j = 1; j <= n; j++) arr[i][j] = r.nl();
                }

                long[][] dp1 = new long[n + 2][2];
                long[][] dp2 = new long[n + 2][2];
                for (int i = 1; i <= n; i++) {
                    dp1[i][0] = arr[0][i] + dp1[i - 1][0];
                    dp2[i][0] = arr[1][i] + dp2[i - 1][0];
                }
                for (int i = n; i >= 1; i--) {
                    dp1[i][1] = arr[0][i] + dp1[i + 1][1];
                    dp2[i][1] = arr[1][i] + dp2[i + 1][1];
                }
                ...
                List<Pair> list = new ArrayList<>();
                {
                    ...
                    if (i <= n) {
                        do {
                            long ele = dp1[i][0];
                            ele += (dp2[i][1]);
                            long max = Math.max(dp1[i + 1][1], dp2[i - 1][0]);

                            list.add(new Pair(ele, max));
                            i++;
                        } while (i <= n);
                    }
                }
                ...
                for (int i = 0, listSize = list.size(); i < listSize; i++) {
                    Pair ele = list.get(i);
                    count2 = Math.min(count2, ele.second);
                }

                out.write((count2 + " ").getBytes());
                out.write(("\n").getBytes());
