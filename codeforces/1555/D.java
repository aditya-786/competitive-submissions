    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (String ele : list) {
                    StringBuilder current = new StringBuilder(ele);
                    while (current.length() <= n) current.append(ele);
                ...
                while (i < strings.size()) {
                    ...
                    while (j < n) {
                        if (j != 0) {
                            if (sb.charAt(j) == strings.get(i).charAt(j)) {
                                dp[i][j] = 0;
                            } else {
                                dp[i][j] += 1;
                            }
                            ...
                            dp[i][j] = sb.charAt(j) != strings.get(i).charAt(j) ? 1 : 0;
                        }
                        j++;
                    }
                    i++;
                ...
                while (m-- > 0) {
                    ...
                    int j = 0;
                    while (j <= 5) {
                        ans = left == 0 ? Math.min(ans, dp[j][right]) : Math.min(ans, dp[j][right] - dp[j][left - 1]);
                        j++;
                    }
