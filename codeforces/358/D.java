    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int[] a = new int[n + 1];
                for (int i = 1; i <= n; i++) a[i] = r.ni();

                int[] b = new int[n + 1];
                for (int i = 1; i <= n; i++) b[i] = r.ni();

                int[] c = new int[n + 1];
                for (int i = 1; i <= n; i++) c[i] = r.ni();
                ...
                for (int i = 1; i <= n; i++) {
                    ...
                    dp[i][0] = Math.max(a[i] + second, b[i] + first);
                    dp[i][1] = Math.max(b[i] + second, c[i] + first);
                }

                out.write((dp[n][0] + " ").getBytes());
