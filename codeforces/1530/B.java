    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (char[] ele : c) Arrays.fill(ele, '0');

                for (int i = 0; i < m; i += 2) c[0][i] = '1';
                for (int i = 2; i < n; i += 2) c[i][m - 1] = '1';
                for (int i = m - 3; i >= 0; i -= 2) c[n - 1][i] = '1';
                for (int i = n - 3; i >= 2; i -= 2) c[i][0] = '1';

                for (char[] ele : c) {
                    for (char e : ele) out.write((e + "").getBytes());
                    out.write(("\n").getBytes());
                }
                out.write(("\n").getBytes());
            }
