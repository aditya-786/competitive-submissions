    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i=0;i<n;i++) list1.add(r.nl());
                for (int i=0;i<n;i++) list2.add(r.nl());
                ...
                while (high>low){
                    long mid = (low+high)>>1;
                    long pos = (n+mid)-(n+mid)/4;

                    long sum2 = IntStream.iterate(0, i -> i < Math.min(n, pos), i -> i + 1).mapToLong(list2::get).sum();
                    long sum1 = mid*100;
                    sum1 += IntStream.iterate(0, i -> i < Math.min(n, pos - mid), i -> i + 1).mapToLong(list1::get).sum();

                    if (sum1 <= sum2) {
                        if (sum1<sum2) low=mid+1;
                        else{
                            low = mid;
                            high = mid;
                        }
