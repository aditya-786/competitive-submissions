        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i = 0; i < n; i++) {
                    long ele = r.nl();
                    arr[i] = ele;
                    map.put((int) ele, map.getOrDefault((int) ele, 0L) + 1);
                }

                int maxn = (int)(1e5)+5;
                long[][] dp = new long[maxn][2];
                dp[1][0] = map.getOrDefault(1,0L);
                dp[1][1] = 0;

                for (int i = 2; i < maxn; i++) {
                    dp[i][0] = dp[i - 1][1] + ((long)i * map.getOrDefault(i,0L));
                    dp[i][1] = Math.max(dp[i - 1][0], dp[i - 1][1]);
                }

                out.write((Math.max(dp[maxn-1][0], dp[maxn-1][1]) + " ").getBytes());
