    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int[][] arr = new int[n][m];
                for (int i = 0; i < n; i++)
                    for (int j = 0; j < m; j++) arr[i][j] = r.ni();
                ...
                while (high - low > 1) {
                    ...
                    for (int i = 0; i < n; i++) {
                        int mask = 0;
                        for (int j = 0; j < m; j++) mask |= (arr[i][j] >= mid ? 1 : 0) << j;
                        present[mask] = 1;
                        pos[mask] = i;
                    }
                    boolean ff = false;
                    for (int i = 0; i < 1 << m; i++)
                        for (int j = 0; j < 1 << m; j++)
                            if ((i | j) == (1 << m) - 1 && present[i] > 0 && present[j] > 0) {
                                pos1 = pos[i];
                                pos2 = pos[j];
                                ff = true;
                            }
                ...
                out.write((++pos1 + " ").getBytes());
                out.write((++pos2 + " ").getBytes());
                out.write(("\n").getBytes());
            }
            // Solution Ends Here
        } catch (IOException e) {
