...
public class Main {
    ...
    static int get(int x, int y, int[] a) {
        return Math.max(0, a[x] - a[y] + 1);
    }
    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                Arrays.fill(dp1[n - 1], inf);
                dp1[n - 1][0] = dp1[n - 1][1] = 0;
                Arrays.fill(dp2[n - 1], inf);
                dp2[n - 1][0] = 0;

                for (int i = n - 2; i >= 0; i--) {
                    for (int k = 1; k <= m; k++) {
                        dp2[i][k] = Math.min(dp2[i + 1][k], dp1[i + 1][k] + get(i, i + 1, arr));
                        if (i == n - 2) {
                            if (k > 1) dp1[i][k] = inf;
                            else dp1[i][k] = get(i + 1, i, arr);
                        } else {
                            int min1 = dp2[i + 2][k - 1] + get(i + 1, i, arr);
                            int getMax = Math.max(get(i + 1, i, arr), get(i + 1, i + 2, arr));
                            int min2 = dp1[i + 2][k - 1] + getMax;
                            dp1[i][k] = Math.min(min1, min2);
                        }
                    }
                }

                for (int k = 1; k <= m; k++) out.write((Math.min(dp1[0][k], dp2[0][k]) + " ").getBytes());
