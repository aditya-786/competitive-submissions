...
public class Main {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                if ((a < 0 || b < 0) && (a >= 0 || b < 0)) {
                    ...
                    top = Math.max(ac1, top);
                    out.write((top + " ").getBytes());
                } else {
                    for (int i = 0; i < n; i++) {
                        count += (a + b);
                    }
    ...
    static long lower_bound(List<Long> a, long x) {
        int l = -1, r = a.size();
        while (l + 1 < r) {
            int m = (l + r) >>> 1;
            if (a.get(m) >= x) r = m;
