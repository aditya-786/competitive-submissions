        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    for (int pos = 0; pos < m; pos++) if (s[pos] == '1') list.add(pos);

                    for (int j = 0; j <= k; j++) {
                        if (j >= list.size()) pass[j] = 0;
                        else {
                            pass[j] = m;
                            for (int p = 0; p <= j; p++) {
                                int val = list.get(list.size() - p - 1) - list.get(j - p) + 1;
                                pass[j] = Math.min(pass[j], val);
                            }
                        }
                    }
                    ...
                    for (int j = k; j >= 0; j--) {
                        int res = n * m;
                        for (int p = 0; p <= j; p++) res = Math.min(res, pass[p] + dp[j - p]);
                        dp[j] = res;
                    }
                }

                out.write((dp[k] + " ").getBytes());
