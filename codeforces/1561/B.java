...
public class Main {
    ...
    static long min(long a, long b, long x, long y) {
        return Math.abs(x - a);
    }

    static long max(long a, long b, long x, long y) {
        if (y > a) return a + x;
        else if (y < a) return y + b;
        return x + y;
    }
    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                long x = n >> 1;
                long y = (n + 1) >> 1;
                long ans1 = min(a, b, x, y);
                long ans2 = max(a, b, x, y);

                for (long i = ans1; i <= ans2; i+=2) ans.add(i);
                ...
                for (long i = ans1; i <= ans2; i += 2) ans.add(i);
                out.write((ans.size() + " ").getBytes());
                out.write(("\n").getBytes());
                for (long ele : ans) out.write((ele + " ").getBytes());
                out.write(("\n").getBytes());
            }
            // Solution Ends Here
        } catch (IOException e) {
