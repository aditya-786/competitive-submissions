            ...
            while (testcases-- > 0) {
                ...
                for (int i = 0; i < k; i++) {
                    ...
                    for (int j = 0; j < maxn; j++)
                        if (dp[pos][j] > 0) {
                            long ele = (dp[pos][j] + 1 - present) >> 1;
                            dp[pos == 0 ? 1 : 0][j ^ x] += ele;
                            dp[pos == 0 ? 1 : 0][j] += dp[pos][j] - ele;
                            present = (present + dp[pos][j]) & 1;
                            dp[pos][j] = 0;
                        }
                    pos = pos == 0 ? 1 : 0;
                }
                ...
                int min = 0;
                while (dp[pos][min] <= 0) min++;
                int max = maxn - 1;
                while (dp[pos][max] <= 0) max--;
