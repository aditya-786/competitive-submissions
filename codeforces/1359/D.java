    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int maxe1 = 100010;
                int maxe2 = 61;
                int[][] dpL = new int[maxe1][maxe2];
                int[][] dpR = new int[maxe1][maxe2];

                for (int i = 1; i <= n; i++) {
                    int ele = arr[i] + 30;
                    for (int j = ele; j <= 60; j++) {
                        dpL[i][j] = Math.max(0, dpL[i - 1][j] + ele - 30);
                    }
                }
                ...
                for (int i = n; i > 0; i--) {
                    int ele = arr[i] + 30;
                    for (int j = ele; j <= 60; j++) {
                        dpR[i][j] = Math.max(0, dpR[i + 1][j] + ele - 30);
                    }
                }

                int ans = 0;
                for (int i = 1; i <= n; i++) {
                    int left = dpL[i - 1][arr[i] + 30];
                    int right = dpR[i + 1][arr[i] + 30];
