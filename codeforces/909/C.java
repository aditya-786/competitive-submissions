    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int n = r.ni();
                int[] arr = new int[n];
                for (int i = 0; i < n; i++) arr[i] = r.word().charAt(0) == 'f' ? 1 : 0;
                ...
                for (int i = 0; i < n; i++) {
                    if (i != 0 && arr[i - 1] != 0) {
                        for (int j = 0; j < n; j++) dp[i + 1][j + 1] = (dp[i + 1][j + 1] + dp[i][j]) % MOD;
                    } else {
                        for (int j = 0; j <= n; j++) dp[i + 1][j] = (dp[i + 1][j] + dp[i][j]) % MOD;
                        for (int j = n - 1; j >= 0; j--) dp[i + 1][j] = (dp[i + 1][j] + dp[i + 1][j + 1]) % MOD;
                    }
                }
