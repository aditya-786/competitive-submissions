...
//                        for (int pos = i; !(pos < 0 || pos < i - p); pos--) {
//                            sum = (sum + arr[pos]) % p;
//                            dp[i + 1][j] = Math.max(dp[i + 1][j], dp[pos][j - 1] + sum);
//                        }

                        for (int ele : pos) {
                            sum = ((dp[i + 1][0] - dp[ele][0]) % p + p) % p;
                            dp[i + 1][j] = Math.max(dp[i + 1][j], dp[ele][j - 1] + sum);
                        }
                    }
                    pos[dp[i + 1][0]] = i + 1;
                }

                out.write((dp[n][k - 1] + " ").getBytes());
