    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int[][] dp = new int[n * k + 100][n * k + 100];
                ...
                for (int i = 1; i <= k; i++) h[i] = r.ni();

                int ans = 0;
                for (Map.Entry<Integer, Integer> entry : map2.entrySet()) {
                    int first = entry.getKey();
                    int second = entry.getValue();
                    int size = map1.getOrDefault(first, 0);
                    //for (int[] val : dp) Arrays.fill(val, 0);
                    for (int i = 1; i <= second; i++) {
                        for (int j = Math.min(size, i * k); j > 0; j--) {
                            for (int t = Math.min(k, j); t >= 0; t--) {
                                dp[i][j] = Math.max(dp[i][j], dp[i - 1][j - t] + h[t]);
                            }
                        }
                    }
                    ans += dp[second][Math.min(size, second * k)];
                }
