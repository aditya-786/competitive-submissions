...
public class Main {
    ...
    static void dfs(int src, int dest) {
        ...
        for (Pair pair : graph[src]) {
            int vertex = pair.first;
            int weight = pair.second;

            if (vertex != dest) {
                dfs(vertex, src);
                long fuelLeft = maxFuel[vertex] - weight;
                if (fuelLeft > max1) {
                    max2 = max1;
                    max1 = fuelLeft;
                } else if (fuelLeft > max2) max2 = fuelLeft;
            }
        }

        maxVal = Math.max(maxVal, max1 + max2 + gases[src]);
        maxFuel[src] = max1 + gases[src];
    }
    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                gases = new long[n + 1];
                maxFuel = new long[n + 1];

                for (int i = 1; i <= n; i++) gases[i] = r.nl();
