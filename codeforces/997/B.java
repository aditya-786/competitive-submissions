    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                long[][] dp = new long[5005][105];
                dp[0][0] = 1;

                for (int i = 1; i <= 50 * m; i++) {
                    for (int j = 1; j <= m; j++) {
                        if (i >= 50) dp[i][j] |= dp[i - 50][j - 1];
                        if (i >= 10) dp[i][j] |= dp[i - 10][j - 1];
                        if (i >= 5) dp[i][j] |= dp[i - 5][j - 1];
                        if (i >= 1) dp[i][j] |= dp[i - 1][j - 1];
                    }
                }

                long count = IntStream.rangeClosed(1, 50 * m).filter(i -> dp[i][m] > 0).count();

                out.write(((m == n ? count : (count + 49L * (n - m))) + " ").getBytes());
