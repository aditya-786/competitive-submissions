    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i = 0; i <= n; i++) for (int j = 0; j <= maxn; j++) dp[i][j] = Integer.MIN_VALUE;
                dp[0][0] = 0;

                for (int i = 1; i <= n; i++) {
                    for (int j = n; j > 0; j--) {
                        for (int k = 0; k <= maxn; k++)
                            dp[j][k] = Math.max(dp[j][k], dp[j - 1][Math.abs(k - a[i])] + b[i]);
                    }
                }

                int get = 0;
                for (int i = 1; i <= n; i++) get += b[i];
                String odd = ".500000000";
                String even = ".000000000";
                for (int i = 1; i <= n; i++) {
                    int ans = 0;
                    for (int j = 0; j <= 100 * n; j++) ans = Math.max(ans, Math.min(get + dp[i][j], j << 1));
                    out.write(((ans >> 1) +""+ ((ans&1) == 1 ? odd : even)+" ").getBytes());
                }
