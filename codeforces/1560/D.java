            ...
            while (testcases-- > 0) {
                ...
                for (String str : list) {
                    ...
                    for (int i = 0; i < str.length(); i++) {
                        ...
                        int step = 0, k = 0;
                        while (step < max) {
                            if (count >= res.length() || res.charAt(count) != s.charAt(step)) {
//                                count+=2;
//                                res.append(s.charAt(step));
                            } else {
                                count++;
                            }
                            step++;
                        ...
                        if (count == res.length()) {
                            int minVal = (max - res.length());
                            int totalLength = (str.length() - res.length());
                            minTrans = Math.min(minVal + totalLength, minTrans);
                        } else {
                            break;
                        }
                    }
                }
                out.write((minTrans + " ").getBytes());
                out.write(("\n").getBytes());
            }
            // Solution Ends Here
        } catch (IOException e) {
