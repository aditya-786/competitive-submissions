...
public class Main {
    ...
    static void permute(StringBuilder s, StringBuilder answer) {
        if (s.length() == 0) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char ch = s.charAt(i);
            StringBuilder left_substr = new StringBuilder(s.substring(0, i));
            StringBuilder right_substr = new StringBuilder(s.substring(i + 1));
            StringBuilder rest = new StringBuilder().append(left_substr).append(right_substr);
            permute(rest, new StringBuilder().append(answer).append(ch));
        }
    }
    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int k = r.ni();
                permute(sb, new StringBuilder(""));
              //  Collections.sort(list);
                List<String> al = new ArrayList<>(list);
                out.write((al.get(k - 1) + " ").getBytes());
            }
            // Solution Ends Here
        } catch (IOException e) {
            e.printStackTrace();
