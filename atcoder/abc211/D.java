...
public class Main {
    ...
    static int BFS(int n, ArrayList<ArrayList<Integer>> graph){
        ...
        while (!queue.isEmpty()){
            int u = queue.poll();
            for (int ele : graph.get(u)){
                if (dist[ele] == Integer.MAX_VALUE){
                    dist[ele] = dist[u]+1;
                    queue.add(ele);
                    paths[ele] = paths[u];
                }else if (dist[ele] == (dist[u]+1)){
                    paths[ele]+=paths[u];
                    paths[ele]%=MOD;
                }
            }
    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i=0;i<m;i++) addEdge(graph, r.ni()-1, r.ni()-1);

                out.write((BFS(n, graph) + " ").getBytes());
