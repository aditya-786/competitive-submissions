    ...
    static void run(){
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases -- > 0){
                ...
                for (int i=0;i<len;i++) list.add(r.nl());

                if (n>1) {
                    List<Long> al = new ArrayList<>();
                    for (int i = 0; i < len; i += 2) al.add(Math.max(list.get(i), list.get(i + 1)));

                    List<Long> temp = new ArrayList<>(al);
...
//                    out.write(("\n").getBytes());
                        al.clear();
                        for (int i = 0; i < len; i += 2) al.add(Math.max(temp.get(i), temp.get(i + 1)));
